  ...
  class App < Sinatra::Base
    ...
    post '/dashboards/:id' do
      ...
      if !settings.auth_token || settings.auth_token == auth_token
        self.class.send_event(params['id'], body, 'dashboards')
    ...
    post '/widgets/:id' do
      ...
      auth_token = body.delete("auth_token")
      if !settings.auth_token || settings.auth_token == auth_token
        self.class.send_event(params['id'], body)
        204 # response without entity body
      else
        status 401
        "Invalid API key\n"
    ...
    Thin::Server.class_eval do
      def stop_with_connection_closing
        Dashing::App.settings.connections.dup.each(&:close)
    ...
    def self.send_event(id, body, target=nil)
      body[:id] = id
      body[:updatedAt] ||= Time.now.to_i
      event = format_event(body.to_json, target)
      Dashing::App.settings.history[id] = event unless target == 'dashboards'
      Dashing::App.settings.connections.each { |out| out << event }
    end

    def self.format_event(body, name=nil)
      str = ""
      str << "event: #{name}\n" if name
      str << "data: #{body}\n\n"
    end
    ...
    def self.require_glob(relative_glob)
